...
const ViewAllLink = styled(Link)`
  ...
  font-size: 0.9rem; font-weight: 600; color: ${({ theme }) => theme.colors.accent};
...
const BlogCover = styled.div`
  ...
  img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.45s cubic-bezier(0.16,1,0.3,1); }
...
const ReadLink = styled(Link)`
  display: inline-flex; align-items: center; gap: 0.3rem;
  font-size: 0.8125rem; font-weight: 600; color: ${({ theme }) => theme.colors.accent};
  transition: gap ${({ theme }) => theme.transitions.fast};
  &:hover { gap: 0.5rem; }
`;
      ...
      <Container>
        <SectionHeader>
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={defaultViewport}>
            <ViewAllLink to="/blog">Tous les articles <ArrowRight size={15} /></ViewAllLink>
          </motion.div>
        </SectionHeader>
        ...
        >
          {latest.map((post) => (
            <BlogCard key={post.id} variants={staggerItem}>
              ...
              <BlogBody>
                <BlogMeta>
                  <CatBadge>{post.category}</CatBadge>
                  <ReadTime><Clock size={11} /> {post.read_time} min</ReadTime>
                </BlogMeta>
                ...
                <BlogFooter>
                  ...
                  <ReadLink to={`/blog/${post.slug}`}>Lire <ArrowUpRight size={12} /></ReadLink>
                </BlogFooter>
              </BlogBody>
            </BlogCard>
          ))}
